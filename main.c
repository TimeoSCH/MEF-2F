...
int main() {
    ...
    } while (team_size < 1 || team_size > MAX_COMBATTANTS);

    Team team1, team2;
    configure_team(&team1, team_size, "Player 1");

    int ai_control = 0;
    if (num_players == 2) {
        configure_team(&team2, team_size, "Player 2");
    } else {
        strcpy(team2.name, "AI");
        team2.member_count = team_size;
        for (int i = 0; i < team_size; i++) {
            team2.members[i] = available_fighters[rand() % total_fighters];
        }
        ai_control = 1;
    }
